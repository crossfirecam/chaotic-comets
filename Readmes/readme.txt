...
Known Possible Bugs
------------------
The game is taking focus of keyboard while in other apps:
...
Due to key rollover on some keyboards, both players using keyboard cannot shoot, thrust and rotate all at the same time. The default controls were kept the same due to the low chance of that specific scenario happening. The controls can be changed in Options.

Note about game not continuing past Level 1:
The game relies on save data to load the shop screen and subsequent levels. If after level 1 is over, you find yourself seeing an error dialog, please contact me to help narrow down why the error is caused.